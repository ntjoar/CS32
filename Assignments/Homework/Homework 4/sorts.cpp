...
//  sorts.cpp
//
//  Created by Nathan Tjoar on 3/2/19.
...
void insertion_sort(vector<Store>& s, bool comp(const Store&, const Store&))
{
    ...
    // final ordering.
    if (s.size() == 2  &&  comp(s[1], s[0])) // Trivial, already done
        swap(s[0], s[1]);
